...
export async function POST(req: NextRequest, res: NextResponse) {
  try {
    ...
    const {
      birthDate, dni, fullName, gender, instagram, mail, phoneNumber, residence, secondaryPhoneNumber, birth,
    } = profile
    ...
    const { error: errorCreate, data: dataCreate } = await fetchClient.POST('/profile/create', {
      headers: {
    ...
    if (errorCreate) {
      ...
      return NextResponse.json({ error: messageString }, { status: errorCreate.statusCode });
    }
